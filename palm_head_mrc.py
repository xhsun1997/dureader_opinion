...
class MRC(Head):
    ...
    def __init__(self, max_query_len, input_dim, pred_output_path=None, verbose=False, with_negative=False, do_lower_case=False, max_ans_len=None, null_score_diff_threshold=0.0, n_best_size=20, phase='train',
                    learning_strategy='pointwise', margin=0.5,dropout_prob=0.2, param_initializer_range=0.02,num_classes=3):
    ...
    def inputs_attrs(self):
        ...
        bb = {"encoder_outputs": [[-1, -1, self._hidden_size], 'float32'],
            "sentence_pair_embedding": [[-1, self._hidden_size], 'float32']}
    ...
    def outputs_attr(self):
        if self._is_training:
            ...
            return {'start_logits': [[-1, -1, 1], 'float32'],
                    ...
                    "logits":[[-1,self._num_classes],"float32"]}
    ...
    def build(self, inputs, scope_name=""):
        ...
        probs_cls = fluid.layers.softmax(logits_cls)

        logits_mrc = fluid.layers.fc(
            input=enc_out,
            size=2,
            num_flatten_dims=2,
            param_attr=fluid.ParamAttr(
        ...
        logits_mrc = fluid.layers.transpose(x=logits_mrc, perm=[2, 0, 1])
        start_logits, end_logits = fluid.layers.unstack(x=logits_mrc, axis=0)
        ...
        if self._is_training:
            ...
            ce_loss=fluid.layers.cross_entropy(inputs=probs_cls,label=labels)
            loss_cls=fluid.layers.mean(x=ce_loss)

            total_loss = (start_loss + end_loss+loss_cls) / 3.0
